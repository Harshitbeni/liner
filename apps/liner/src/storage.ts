...
export function loadInterfaceKitEnabled(): boolean {
  try {
    return localStorage.getItem(INTERFACE_KIT_KEY) === '1';
  } catch {
    return false;
  }
}

export function saveInterfaceKitEnabled(enabled: boolean): void {
  try {
